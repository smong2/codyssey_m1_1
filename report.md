...
- **AI 한계 보완 및 본인 검증 로직:** AI가 최초 생성한 예측 코드는 과적합(Overfitting)으로 인해 잔차(Residual)가 지나치게 0으로 수렴하는 문제(MAPE < 1%)가 있었습니다. 본인은 이를 해결하기 위해 1) 과거 학습 구간과 미래 시뮬레이션 구간의 **Data Split을 명확히 분리 재작성**하고, 2) 잔차 분포 히스토그램 연산 로직을 **AI 도움 없이 수동으로 추가 코딩**하여 모델의 실제 신뢰성을 검증했습니다.
